...
`}function L(o){return o.map(b).join("")}function w(o){const t=L(o);c.insertAdjacentHTML("beforeend",t),h.refresh()}function v(){l.classList.remove("is-hidden")}function S(){l.classList.add("is-hidden")}const P=document.querySelector(".form");P.addEventListener("submit",$);async function $(o){o.preventDefault();const t=o.target.elements["search-text"].value.trim();if(t){y(),v();try{const s=await g(t);w(s.hits)}catch(s){console.error(s)}S(),o.target.reset()}}
